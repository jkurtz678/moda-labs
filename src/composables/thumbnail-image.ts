...
import { getTokenMetaMediumImageURL, type FirestoreDocument, type TokenMeta, getTokenMetaThumbnailImageURL, getTokenMetaAnimationURL } from "@/types/types"
...
export function useTileAnimation(token_meta: Ref<FirestoreDocument<TokenMeta>>) {
    const thumbnail_url = ref(new URL(`../assets/logo.png`, import.meta.url).href);
    // needs to be watchEffect because we want it to trigger initially AND we want it to run again when the props.token_meta changes
    watchEffect(async () => {
        thumbnail_url.value = await getTokenMetaAnimationURL(token_meta.value);
        // if firebase archive thumbnail is not found or animation thumbnail is not found, try again in 12 seconds
        if (thumbnail_url.value.includes("logo.png") || thumbnail_url.value.includes("medium_")) {
            setTimeout(async () => {
                const new_url = await getTokenMetaAnimationURL(token_meta.value);
                if(new_url != thumbnail_url.value) {
                    thumbnail_url.value = new_url;
                }
            }, 12000);
